/**
 * Google Apps Script for TTT CRM CRM Notifications
 * - Sends structured notifications to backend (/api/notify)
 * - Logs entries to a Notifications sheet
 * - Provides follow-up processing helpers
 */

// Configuration
const CONFIG = {
  BACKEND_URL: 'https://TTT CRM-backend-6xzr.onrender.com/api/notify',
  FALLBACK_URL: '',
  // Optional: set a shared secret to authenticate with backend
  // If set, backend must have NOTIFY_SECRET env var with the same value
  BACKEND_SECRET: 'YOUR_SECRET', // Set to match backend NOTIFY_SECRET
...
function notifyCRMAppOnEdit(e) {
  try {
    ...
    if (sheetName.toLowerCase() === 'blackboard') {
...
function sendPushNotificationWithRetry(notification, attempt) {
  ...
  try {
    const pushData = {
      ...
      icon: '/icons/notification-icon.jpg',
      badge: '/icons/notification-icon.jpg',
...
function sendToBackend(notification) {
  try {
    ...
    const options = {
      ...
      headers: (CONFIG.BACKEND_SECRET && CONFIG.BACKEND_SECRET.length)
        ? { 'x-TTT CRM-secret': CONFIG.BACKEND_SECRET }
        : {},
...
/** Stubs for push delivery strategies */
...
/** Process pending scheduled notifications (example) */
